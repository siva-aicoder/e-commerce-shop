...
const ProductDetail = ({ addToCart }) => {
  ...
  const salePrice = product ? getSalePrice(product) : 0;
          ...
          <div className="mb-6">
            {hasDiscount(product) ? (
              <div className="flex items-center">
                ...
                <span className="text-xl text-gray-500 line-through ml-2">₹{basePrice}</span>
                <span className="ml-4 bg-gray-200 text-black px-2 py-1 rounded text-sm">
                  {Math.round((1 - salePrice / basePrice) * 100)}% OFF
                </span>
              </div>
            ) : (
              <span className="text-3xl font-bold text-black">₹{salePrice}</span>
